  ...
  Widget build(BuildContext context) {
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, i) {
        ...
        return Card(
          child: ListTile(
            leading: CircleAvatar(backgroundColor: AppColors.orange.withValues(alpha: 0.15), child: Text('#${v.id.toString().substring(v.id.toString().length - 4)}', style: const TextStyle(color: AppColors.orange, fontSize: 9))),
            title: Text('${formatMoney(v.totalUsd)} · ${v.metodo}', style: const TextStyle(color: Colors.white)),
            subtitle: Text(
              '${v.fecha.day}/${v.fecha.month} ${v.fecha.hour.toString().padLeft(2, '0')}:${v.fecha.minute.toString().padLeft(2, '0')} · ${v.items.length} items · ${v.empleadoNombre}',
