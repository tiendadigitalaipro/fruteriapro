...
import 'merma/merma_screen.dart';
...
final List<AppModule> appModules = [
  ...
  AppModule(id: 'merma', label: 'Merma', icon: Icons.eco, builder: (_) => const MermaScreen()),
  ...
  AppModule(id: 'empleados', label: 'Empleados', icon: Icons.badge, builder: (_) => const EmpleadosScreen(), hasOwnFab: true),
  AppModule(id: 'avances', label: 'Avances', icon: Icons.payments, builder: (_) => const AvancesScreen()),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Row(
          children: [
            const Icon(Icons.local_grocery_store, color: AppColors.orange),
            const SizedBox(width: 8),
            const Text('Frutería Pro'),
            const SizedBox(width: 8),
          ...
          children: [
            const DrawerHeader(
              ...
              child: Column(
                ...
                children: [
                  Icon(Icons.local_grocery_store, color: AppColors.orange, size: 36),
                  SizedBox(height: 8),
                  Text('Frutería Pro', style: TextStyle(color: Colors.white, fontSize: 20, fontWeight: FontWeight.bold)),
            ...
            for (var i = 0; i < appModules.length; i++)
              ListTile(
                leading: Icon(appModules[i].icon, color: i == _selectedIndex ? AppColors.orange : Colors.white70),
                title: Text(appModules[i].label, style: TextStyle(color: i == _selectedIndex ? AppColors.orange : Colors.white70, fontWeight: i == _selectedIndex ? FontWeight.bold : FontWeight.normal)),
      ...
      floatingActionButton: current.hasOwnFab ? null : FloatingActionButton(backgroundColor: AppColors.orange, foregroundColor: Colors.black, onPressed: () => _selectModule(1), tooltip: 'Nueva venta', child: const Icon(Icons.point_of_sale)),
    );
  }
}
