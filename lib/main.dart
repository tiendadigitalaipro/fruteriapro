...
void main() async {
  ...
  runApp(const FruteriaProApp());
}

class FruteriaProApp extends StatelessWidget {
  const FruteriaProApp({super.key});
  ...
  Widget build(BuildContext context) {
    return MultiProvider(
      providers: [
        ...
        ChangeNotifierProvider(create: (_) => MermaController()..cargar()),
  ...
  Widget build(BuildContext context) {
    ...
    switch (licencia.estado) {
      case EstadoLicencia.cargando:
        return const Scaffold(backgroundColor: AppColors.background, body: Center(child: CircularProgressIndicator(color: AppColors.orange)));
