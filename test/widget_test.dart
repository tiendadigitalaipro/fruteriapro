...
import 'package:fruteriapro/data/app_database.dart';
import 'package:fruteriapro/main.dart';
import 'package:fruteriapro/services/license_service.dart';
...
void main() {
  ...
  testWidgets('Sin licencia guardada, pide un código en vez de dar acceso libre', (WidgetTester tester) async {
    ...
    await tester.pumpWidget(const FruteriaProApp());
  ...
  testWidgets('Con licencia PRO guardada, entra directo al dashboard', (WidgetTester tester) async {
    SharedPreferences.setMockInitialValues({'fp_licencia_key': '"PRO-TEST-0000"'});
    LicenseService.client = MockClient((request) async => http.Response(jsonEncode({'valid': true, 'type': 'pro'}), 200));

    await tester.pumpWidget(const FruteriaProApp());
    await tester.pumpAndSettle();

    expect(find.text('Frutería Pro'), findsOneWidget);
